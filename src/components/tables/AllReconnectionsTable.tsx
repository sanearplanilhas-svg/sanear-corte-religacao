...
export default function AllReconnectionsTable() {
  ...
  async function load() {
    ...
    let query = supabase
      ...
      .select("id, matricula, bairro, rua, numero, ponto_referencia, prioridade, status, pdf_ordem_path, ativa_em");
    ...
    query = query.order("ativa_em", { ascending: false });
          ...
          <tbody className="text-slate-200">
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-white/5">
                ...
                <td className="py-2 text-center">
                  {r.ativa_em
                    ? new Date(r.ativa_em).toLocaleString("pt-BR")
                    : "—"}
                </td>
              </tr>
            ))}
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={deleteMode ? 9 : 8} className="py-6 text-center text-slate-400">
